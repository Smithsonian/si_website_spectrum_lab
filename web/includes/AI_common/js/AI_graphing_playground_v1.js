    ...
    if (upperx != []) {
        ...
        for (var i = 0; i < upperx.length; i++) {
            ...
            if (uppery[i] <= 1) {
                ...
                var line_spectrum =
                    document.getElementById('line_spectrum').selected; //Added by Aladdin
        ...
        for (var pxl = 0; pxl < upperx.length; pxl++) {
            ...
            if (Pxl_X_nowC == Pxl_X_next) {
                Pxl_Alpha_Add =
                    (parseFloat(Pxl_Alpha_now) + parseFloat(Pxl_Alpha_next)) /
                    2;
                ...
                for (var pt = 2; pt < 20; pt++) {
                    ...
                    Pxl_X_after = parseInt(
                        (1000 * upperx[pxl + pt] - 200) * pxl_stretch
                    );
                    Pxl_Alpha_next = parseFloat(uppery[pxl + pt]);
                    pxl = pxl + pt;
                    if (Pxl_X_nowC == Pxl_X_after) {
                        Pxl_Alpha_Add =
                            (parseFloat(Pxl_Alpha_Add) +
                                parseFloat(Pxl_Alpha_next)) /
                            Pxl_count;
...
function drawXAxis(W_CTX, W_CAN, xStretch) {
    ...
    for (i = 0; i < 60; i++) {
        if (legend_yAxisOffset + pxl_stretch * (800 + 1000 * i) < W_CAN.width) {
            W_CTX.moveTo(
                legend_yAxisOffset + pxl_stretch * (800 + 1000 * i),
                10
            );
            W_CTX.lineTo(
                legend_yAxisOffset + pxl_stretch * (800 + 1000 * i),
